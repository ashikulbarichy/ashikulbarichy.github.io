...
import type { MenuData, UiText } from '@/lib/types';
...
/**
 * Projects are server-fetched and passed in whole. Category filtering happens
 * client-side over that array, so it stays instant and needs no refetch, while
 * the full list is still present in the server-rendered HTML for crawlers.
 */
...
const ProjectsGrid = ({
  ...
  const stats = useMemo(
    () => ({
      ...
      totalTechnologies: new Set(allProjects.flatMap((p) => p.tags)).size,
    }),
    [allProjects]
  );
    ...
    return (
      <section className="relative w-full h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-500 font-mono text-xs uppercase tracking-widest">{ui.projects.loadingLabel}</p>
        </div>
      </section>
    );
  }
      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-between items-center px-6 py-6 sm:px-10 sm:py-8 md:px-12 md:py-8 pointer-events-none">
        ...
        <div className="pg-header-item opacity-0 pointer-events-auto">
          ...
          >
            ...
            <span className="text-zinc-500 group-hover:text-white transition-colors duration-300 inline-block group-hover:translate-x-0.5 transform duration-300">{menuData.brandLast}</span>
          </Link>
        </div>
        ...
        <div className="pg-header-item opacity-0 pointer-events-auto flex items-center gap-6">
          <div className="hidden sm:flex gap-5 text-[10px] font-mono text-zinc-600 uppercase tracking-widest">
            <span>[ {stats.totalProjects} {ui.projects.statsProjectsLabel} ]</span>
            <span>[ {stats.totalTechnologies} {ui.projects.statsTechsLabel} ]</span>
          </div>
          <div 
            className="flex flex-col gap-1.5 cursor-pointer group" 
            onClick={() => setIsMenuOpen(true)} 
            title={ui.hero.openMenuLabel}
          >
          ...
          >
            ...
            {projects.length === 0 && (
              <div className="absolute inset-0 flex items-center justify-center">
                <p className="font-mono text-xs uppercase tracking-widest text-zinc-700">{ui.projects.emptyMessage}</p>
              </div>
            )}
          </div>
          ...
          {current ? (
            <div className="flex-1 flex flex-col justify-center items-end gap-6 py-6 w-full ml-auto">
              ...
              <div className="pg-detail-content w-full">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-zinc-600 block mb-1">{ui.projects.categoryLabel}</span>
              ...
              <div className="pg-detail-content w-full flex flex-col items-end gap-2">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-zinc-600">{ui.projects.techStackLabel}</span>
              ...
              {/* Overview */}
              <div className="pg-detail-content w-full flex flex-col items-end gap-2">
                <span className="font-mono text-[9px] uppercase tracking-[0.2em] text-zinc-600">{ui.projects.overviewLabel}</span>
              ...
              <div className="pg-detail-content flex gap-5 items-center justify-end flex-wrap">
                {/* Internal link to the project's own page. Without this the
                    per-project pages would be orphaned, reachable only via the
                    sitemap, which is a much weaker discovery signal. */}
                <Link
                  href={`/projects/${current.slug}`}
                  className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-zinc-400 hover:text-white transition-colors border-b border-zinc-700 hover:border-white pb-0.5 font-mono group"
                >
                  <span>{ui.projects.caseStudyLabel}</span>
                ...
                {current.githubUrl && (
                  ...
                  >
                    <span>{ui.projects.githubLabel}</span>
                ...
                {current.projectUrl && (
                  <a
                    ...
                    className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-widest text-zinc-500 hover:text-white transition-colors border-b border-zinc-800 hover:border-white pb-0.5 font-mono group"
                  >
                    <span>{ui.projects.liveSiteLabel}</span>
