...
import type { LandingPageData, MenuData, UiText } from '@/lib/types';
 ...
 * Hero.
 *
 * Data arrives as props from the server component that renders this, rather
 * than being fetched in an effect. That is the whole point of the migration:
 * props are available during server rendering, so the name, role and subtitle
 * are present in the initial HTML where a crawler that does not run
...
const Hero = ({ landingData, menuData, ui }: HeroProps) => {
    ...
    <section ref={heroRef} className="min-h-screen w-full flex flex-col md:flex-row relative bg-black overflow-hidden">
      ...
      >
        ...
        <div className="hero-right-fade-stagger opacity-0 my-auto flex flex-col space-y-6 sm:space-y-8 max-w-xl text-right items-end ml-auto w-full py-8 md:py-12 relative z-10">
          ...
          <p className="text-2xl sm:text-3xl lg:text-4xl font-garamond text-zinc-400 leading-[1.25] font-light tracking-tight hover:text-zinc-200 transition-colors duration-500 select-none cursor-default">
        ...
        <div className="hero-right-fade-stagger opacity-0 flex flex-col sm:flex-row gap-4 sm:gap-6 pt-4 justify-end w-full pb-8 md:pb-12 relative z-10">
          ...
          >
            <span>{ui.hero.viewWorkLabel}</span>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-zinc-400 hover:text-white transition-colors border-b border-zinc-800 hover:border-white pb-1 font-mono cursor-pointer group"
          >
            <span>{ui.hero.downloadCvLabel}</span>
      ...
      <NavMenu
        ...
        closeLabel={ui.nav.closeMenuLabel}
      />
