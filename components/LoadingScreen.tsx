...
import { defaultUiText } from '@/lib/sanity/fallbacks';
import './LoadingScreen.css';

interface LoadingScreenProps {
  onComplete: () => void;
  /** Editable copy from Sanity uiText. Defaults keep the shipped wording. */
  labels?: UiText['loadingScreen'];
}

const LoadingScreen = ({ onComplete, labels = defaultUiText.loadingScreen }: LoadingScreenProps) => {
  ...
  const handleBypass = () => {
    ...
    if (bypassRef.current) {
      ...
      bypassRef.current.innerText = labels.bypassingLabel;
    }
  ...
  const getStatusText = () => {
    if (progress < 30) return labels.stage1;
    if (progress < 65) return labels.stage2;
    if (progress < 90) return labels.stage3;
    if (progress < 100) return labels.stage4;
    return labels.stageComplete;
  };
  ...
  return (
    ...
    >
      <div className="loader-center-box">
        ...
        >
          {progress < 100 ? labels.bypassLabel : labels.grantedLabel}
        </button>
